...
var buildPackage = function(obj, callback) {
  ...
  var rootDir = path.dirname(fs.realpathSync(__filename)),
    definitionFileList = files(path.join(rootDir, "schemas", "definitionFiles", schemaDefinition)).map(function(file) {
      return {
        name: file,
      ...
      fse.outputFile(path.join(obj.source, "metadata.xml"), metadata(obj), function(err) {
        if (err) {
          ...
          if (obj.package.zip) {
            ...
            var shouldUseNameAsIs = Boolean(obj.package.useNameAsIs);
            var zipOutput = path.join(
              obj.package.outputFolder,
              shouldUseNameAsIs
                ? obj.package.name + ".zip"
                : utils.cleanAndTrim(obj.package.name) + "_v" + obj.package.version + "_" + obj.package.date + ".zip"
            );
